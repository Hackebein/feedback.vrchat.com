...
## Canny wake poller (notifications + new posts)

Host timer `feedback-canny-wake.timer` (≈every 1 min) SSO-polls Canny
`/api/notifications/get` and public newest listings. When it sees new
...
3. `sync_runtime_stack.sh` installs/enables the timer via
...
```bash
systemctl status feedback-canny-wake.timer
journalctl -u feedback-canny-wake.service -n 50
```
