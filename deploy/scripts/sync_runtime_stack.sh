...
if [[ -f /etc/feedback-search/notify.env ]]; then
  # Propagate auto-detected Canny scraper user id into notify.env when present.
  if [[ -f "${REPO}/scraper_user_id" ]]; then
    sid="$(tr -d '[:space:]' <"${REPO}/scraper_user_id" || true)"
    if [[ -n "${sid}" ]]; then
      if grep -q '^SCRAPER_CANNY_USER_ID=' /etc/feedback-search/notify.env; then
        sed -i "s|^SCRAPER_CANNY_USER_ID=.*|SCRAPER_CANNY_USER_ID=${sid}|" /etc/feedback-search/notify.env
      else
        printf 'SCRAPER_CANNY_USER_ID=%s\n' "${sid}" >>/etc/feedback-search/notify.env
      fi
    fi
  fi
