...
// snapshotMinBaseline is the smallest established post_state size that can
// trigger the collapse guard. Smaller baselines (early seed) may shrink without
// implying a search outage.
const snapshotMinBaseline = 50

// snapshotMaxVanish is the most posts that may disappear from a complete
// snapshot in one tick before we treat the result as untrustworthy. Real
// deletions are far below this; a search glitch can drop tens of thousands.
const snapshotMaxVanish = 1000
...
// stripScraperVoter removes the SSO scrape bot from the hit's voter list and
// decrements score when that voter was present so notify diffs never announce
// the bot's own upvotes.
func stripScraperVoter(hit *gwHit, scraperID string) {
	if hit == nil || scraperID == "" {
		return
	}
	filtered := hit.Voters[:0]
	removed := false
...
func (d *Dispatcher) processSnapshot(ctx context.Context, sub Subscription, tick int64) error {
	...
	// A complete-but-empty (or near-empty) snapshot after a large baseline is
	// almost always a transient search glitch. Trusting it would wipe post_state
	// and then replay the whole catalog as New post once the index recovers.
	if snapshotBaselineCollapsed(complete, len(prev), len(hits)) {
		err := fmt.Errorf("snapshot collapsed: sub=%d prev=%d hits=%d (refusing baseline wipe)",
			sub.ID, len(prev), len(hits))
	...
	for _, raw := range hits {
		...
		stripScraperVoter(&hit, d.cfg.ScraperUserID)
		pid := hit.id()
		if pid == "" {
			continue
		}
...
// snapshotBaselineCollapsed reports whether a complete snapshot shrank so far
// below an established baseline that it must not be used to reconcile deletions
// or advance post_state.
func snapshotBaselineCollapsed(complete bool, prevCount, hitCount int) bool {
	if !complete || prevCount < snapshotMinBaseline {
		return false
	}
	// Empty results after a real baseline are never trusted (covers scoped
	// filters smaller than snapshotMaxVanish).
	if hitCount == 0 {
		return true
	}
	return prevCount-hitCount > snapshotMaxVanish
}
